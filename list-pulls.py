...
GHMETA = os.getenv('GHMETA', '../aixcoin-gh-meta')
DEFAULT_REPO = os.getenv('DEFAULT_REPO', 'aixcoin/aixcoin')

# == Label to category mapping ==
# See: https://github.com/aixcoin/aixcoin/labels
...
def tests():
    ...
    assert(FQId.parse('aixcoin/aixcoin#1234', 'aixcoin/aixcoin') == FQId('aixcoin', 'aixcoin', 1234))
    assert(FQId.parse('aixcoin-core/gui#1235', 'aixcoin/aixcoin') == FQId('aixcoin-core', 'gui', 1235))
    assert(FQId.parse('#1236', 'aixcoin/aixcoin') == FQId('aixcoin', 'aixcoin', 1236))
    assert(FQId.parse('1237', 'aixcoin/aixcoin') == FQId('aixcoin', 'aixcoin', 1237))
    assert(str(FQId('aixcoin', 'aixcoin', 1239)) == 'aixcoin/aixcoin#1239')
    assert(FQId('aixcoin', 'aixcoin', 1239) < FQId('aixcoin', 'aixcoin', 1240))
    assert(not (FQId('aixcoin', 'aixcoin', 1240) < FQId('aixcoin', 'aixcoin', 1239)))
    assert(FQId('aixcoin', 'aixcoin', 1240) < FQId('aixcoin-core', 'gui', 1239))
    assert(not (FQId('aixcoin-core', 'gui', 1239) < FQId('aixcoin', 'aixcoin', 1240)))
...
def parse_commit_message(msg):
    ...
    for line in msg.splitlines():
        if line.startswith('Github-Pull:'):
            param = line[12:].strip()
            if param.startswith('#'): # compensate for incorrect #aixcoin-core/gui#148
                param = param[1:]
