...
DEFAULT_CONFIG = {
    ...
    'meta': {'aixcoin/aixcoin': '/path/to/aixcoin-gh-meta'},
...
def draw(notifications):
    ...
    for rec in notifications:
        ...
        if rec.subject.type in {'PullRequest', 'Issue'}:
            # PullRequest: https://api.github.com/repos/aixcoin-core/secp256k1/pulls/875
            # Issue: https://api.github.com/repos/aixcoin/aixcoin/issues/20935
            m = re.match('.*\/([0-9]+)$', rec.subject.url)
            issue = int(m.group(1))
            meta = ghmeta.get((rec.repository.full_name, issue))
            ref_str = str(issue)
        elif rec.subject.type == 'Commit':
            # Commit: https://api.github.com/repos/aixcoin/aixcoin/commits/54ce4fac80689621dcbcc76169b2b00b179ee743
            m = re.match('.*\/([0-9a-f]+)$', rec.subject.url)
            ref_str = m.group(1)
            issue = None
            meta = None
        else:
            # Release: https://api.github.com/repos/aixcoin-core/HWI/releases/34442950
