...
def remove_current_translations():
    ...
    This leaves aixcoin_en.ts untouched.
    '''
...
def contains_aixcoin_addr(text, errors):
    if text is not None and ADDRESS_REGEXP.search(text) is not None:
        errors.append('Translation "%s" contains a aixcoin address. This will be removed.' % (text))
        return True
    return False
...
def postprocess_message(filename, message, xliff_compatible_mode):
    ...
    for translation in translations:
        if translation is None:
            continue
        errors = []
        valid = check_format_specifiers(source, translation, errors, numerus) and not contains_aixcoin_addr(translation, errors)
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((aixcoin_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/aixcoin_locale.qrc', 'w') as f:
